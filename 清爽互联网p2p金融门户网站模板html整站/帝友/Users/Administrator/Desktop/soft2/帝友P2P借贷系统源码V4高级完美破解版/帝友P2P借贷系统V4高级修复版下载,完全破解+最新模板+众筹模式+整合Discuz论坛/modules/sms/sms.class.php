...
class smsClass{	
    ...
    //发送短信方法 
    /*
    * @param Array $data = array("smstype"=>"sms","type"=>"类型","user_id"=>"用户","phone"=>"电话","content"=>"内容","time"=>"发送时间");
	* @return Array
    */
    function sendsSms($data){
       ...
       //echo count($data['user_id']);
        if(is_array($data['user_id'])){//新标提醒设置
            //获取该标信息
            ...
            for($j=0;$j<count($data['user_id']);$j++){  
                ...
                if($result['sex']==1){
                    $result['type_content']=str_replace('先生/女士','先生',$result['type_content']);
                }else if($result['sex']==2){
                    $result['type_content']=str_replace('先生/女士','女士',$result['type_content']);
                }
