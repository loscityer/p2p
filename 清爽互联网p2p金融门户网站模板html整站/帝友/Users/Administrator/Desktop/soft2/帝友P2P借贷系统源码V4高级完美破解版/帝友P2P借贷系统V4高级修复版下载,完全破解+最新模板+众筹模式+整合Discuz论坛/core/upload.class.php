...
$this->file_type = isset($data['file_type'])?$data['file_type']:array('jpg','gif','bmp','png');
 $this->file_size = isset($data['file_size'])?$data['file_size']:"2048"; 
$this->file_quality = isset($data['file_quality'])?$data['file_quality']:"80"; 
$this->file_newname = (isset($data['user_id'])?$data['user_id']:"0")."_".(isset($data['code'])?$data['code']:"system")."_{$data['type']}_".time().rand(0,9);
...
if(strpos($_FILES[$file]['type'],'image')===false && $_FILES[$file]['type']!= '') $error = -4;
if($_FILES[$file]['size'] >$this->file_size*1024) $error = -5; 
