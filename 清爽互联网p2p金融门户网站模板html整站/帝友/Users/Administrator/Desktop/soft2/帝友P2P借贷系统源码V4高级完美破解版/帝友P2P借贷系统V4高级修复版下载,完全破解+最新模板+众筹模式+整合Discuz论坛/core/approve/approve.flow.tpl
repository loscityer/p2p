...
<div class="module_add">
	<div class="module_title"><strong>流转认证</strong></div>
	...
	<div class="module_title"><strong>修改流转认证信息</strong><input type="hidden" name="user_id" value="{$magic.request.user_id}" /></div>
	...
	<div class="module_add">
	<div class="module_title"><strong>流转认证列表</strong></div>
	</div>
<table  border="0"  cellspacing="1" bgcolor="#CCCCCC" width="100%">
	...
	{ list module="approve" function="GetFlowList" var="loop" username=request  realname=request  card_id=request  epage=8 }
	...
	<tr {if $key%2==1} class="tr2"{/if}>
		...
		<td class="main_td1" align="center"><a href="javascript:void(0)" onclick='tipsWindown("流转认证人工审核","url:get?{$_A.query_url_all}&examine={$item.user_id}",500,230,"true","","false","text");'/>审核</a>/<a href="{$_A.query_url_all}&user_id={$item.user_id}&page={$magic.request.page}">修改</a></td>
	</tr>
	{/foreach}
	<tr>
			...
			<div class="floatr">
				用户名：<input type="text" name="username" id="username" size="7" value="{$magic.request.username|urldecode}"/>    <input type="button" value="{$MsgInfo.users_name_sousuo}" / onclick="sousuo()">
			</div>
			</td>
		</tr>
