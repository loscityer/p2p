...
class accountClass{
	...
	function GetList($data = array()){
		...
		$_list = $mysql->db_fetch_arrays(str_replace(array('SELECT', 'SQL','ORDER', 'LIMIT'), array($_select,$_sql,$_order, ''), $sql));
		...
		if (IsExiest($data['type'])!=false) {
			if ($data['style']=="fxc"){
				if ($data['type']==2 && $_REQUEST['type']==""){
					...
					$_sql .= " and p1.type = '{$_REQUEST['type']}'";
				}
			}else{
				if ($data['type']==3){
	...
	function GetCashList($data = array()){
		...
		if (IsExiest($data['excel'])==1){
			$result= $mysql->db_fetch_arrays(str_replace(array('SELECT', 'SQL', 'ORDER', 'LIMIT'), array($_select,'','order by p1.addtime desc,p1.id desc', ""), $sql));
			$title = array("提现金额","手续费","提现时间","备注","状态","管理备注");
			$linkage_result = $_G['linkage'];
			foreach ($result as $key => $value){
				if ($value['status']==1 || $value['status']==0){
					if ($value['status']==0){ $value['status']="审核中"; }elseif ($value['status']==1){$value['status']=" 提现成功";}
					$_data[$key] = array($value['total'],$value['fee'],date("Y-m-d",$value['addtime']),$value['remark'],$value['remark'],$value['status'],$value['verify_remark']);
				}
			}
			self::exportData(date("Y-m-d",time())."提现记录详情",$title,$_data);
			exit;
		}
		//是否显示全部的信息
		elseif (IsExiest($data['limit'])!=false){
	...
	function GetRechargeList($data = array()){
		...
		$_select = "p1.*,p2.username,p3.name as payment_name, p4.realname";
		...
		$sql = "select SELECT from {account_recharge} as p1 
				 left join {users} as p2 on p1.user_id=p2.user_id
				 left join {users_info} as p4 on p4.user_id=p1.user_id
	...
	function VerifyRecharge($data = array()){
		...
		if ($data['status']==1){
			...
			$log_info["frost"] = 0;//冻结金额
			$log_info["await"] = 0;//待收金额
			$log_info["type"] = "recharge_jiangli";//类型
			$log_info["to_userid"] = 0;//付给谁
			$log_info["remark"] = "通过{$payment}充值奖励金额{$recharge_fee}元";//备注
			$result = self::AddLog($log_info);
			}else{
			$log_info["user_id"] = $recharge_userid;//操作用户id
			$log_info["nid"] = "recharge_fee_".$data['nid'];//订单号
	...
	function GetOne($data = array()){
		...
		$_select = "p1.*,(select sum(total) from {account_cash} where user_id=p1.user_id and status=1) as cash_total,(select sum(money) from {account_recharge} where user_id=p1.user_id and status=1) as recharge_total";
	...
	function GetRechargeOne($data = array()){
		...
		$_select = "p1.*";
		$sql = "select $_select from `{account_recharge}` as p1 $_sql";
		$result = $mysql->db_fetch_array($sql);
	...
	function VerifyCash($data = array()){
       ...
       $mysql->db_query($sql);
		...
		if ($data['status']==1){
			...
			$log_info["income"] = 0;//收入
			$log_info["expend"] = 0;//支出
			$log_info["balance_cash"] = $total;//可提现金额
			$log_info["balance_frost"] = 0;//不可提现金额
			$log_info["frost"] = -$total;//冻结金额
			$log_info["await"] = 0;//待收金额
			$log_info["type"] = "cash_false";//类型
			$log_info["to_userid"] = 0;//付给谁
			$log_info["remark"] = "申请{$cash_account}元提现失败，解冻提现金额{$cash_account}元和手续费{$cash_fee}元";//备注
			$result = self::AddLog($log_info);
	...
	function  OnlineReturn ($data = array()){
		...
		$user_id = substr($trade_no,9,strlen($trade_no));
		...
		$rechage_result = self::GetRechargeOne(array("nid"=>$trade_no,"user_id"=>$user_id));
			...
			//测试输出  hummer create 201309082032
			 ...
			 $ka=fopen("2.txt","a+");
		     fwrite($ka,$rechage_result['status']);
		     fclose($ka);
		...
		if($rechage_result['status']==2 && $rechage_result!=false){
				...
				$log_info["type"] = "recharge_fee";//类型
				$log_info["to_userid"] = 0;//付给谁
				$log_info["remark"] = "充值扣除手续费{$rechage_result['fee']}元";//备注
				$result = self::AddLog($log_info);	
	...
	public static function Getborrowjin($data = array()){
		...
		$sql = "select account from `{borrow}`  
                 where user_id='$user_id' and is_jin=1 and (status =1 or (status =3 and repay_account_yes = 0)) ";
		$result = $mysql->db_fetch_array($sql);
		return $result;
	}
