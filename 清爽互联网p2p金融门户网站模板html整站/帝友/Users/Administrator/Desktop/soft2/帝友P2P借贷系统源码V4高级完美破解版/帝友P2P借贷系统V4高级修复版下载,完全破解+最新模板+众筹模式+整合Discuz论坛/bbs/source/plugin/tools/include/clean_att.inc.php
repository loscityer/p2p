...
$mod = in_array($_GET['mod'],array('clean_att','check_file')) ? $_GET['mod'] : 'clean_att';
...
if($mod == 'clean_att'){
	...
	if(submitcheck('att_clean_submit')){
		if(count($_G['gp_attarray']) <= 0) {
			...
			cpmsg($toolslang['clearatt_done'],"action=plugins&operation=$operation&do=$do&identifier=$identifier&pmod=maintain&cp=clean_att&mod=clean_att",'succeed');	
		}
	}
	if(submitcheck('att_submit')){
		...
		if(count($dlist) <= 0) {
			cpmsg($toolslang['clearatt_nolaji'],"action=plugins&operation=$operation&do=$do&identifier=$identifier&pmod=maintain&cp=clean_att&mod=clean_att",'error');	
		}
		...
		showformheader("plugins&operation=$operation&do=$do&identifier=$identifier&pmod=maintain&cp=clean_att&mod=clean_att");
		...
		foreach(glob(DISCUZ_ROOT."/data/attachment/portal/*",GLOB_ONLYDIR) as $dirname){
			$dirlist[1] = array($dirname,str_replace(DISCUZ_ROOT.'/data/attachment/','',$dirname));
		}
		...
		showformheader("plugins&operation=$operation&do=$do&identifier=$identifier&pmod=maintain&cp=clean_att&mod=clean_att",'submit');
