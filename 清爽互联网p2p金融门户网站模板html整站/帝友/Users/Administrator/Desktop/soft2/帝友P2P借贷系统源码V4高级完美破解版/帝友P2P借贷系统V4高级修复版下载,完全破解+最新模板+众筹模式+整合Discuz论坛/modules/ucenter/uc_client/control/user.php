...
class usercontrol extends base {
			...
			if($this->user = $_ENV['user']->get_user_by_uid($uid)) {
				...
				foreach($this->cache['apps'] as $appid => $app) {
					if($app['synlogin'] && $app['appid'] != $this->app['appid']) {
						$synstr .= '<script type="text/javascript" src="'.$app['url'].'/api/uc.php?time='.$this->time.'&code='.urlencode($this->authcode('action=synlogin&username='.$this->user['username'].'&uid='.$this->user['uid'].'&password='.$this->user['password']."&time=".$this->time, 'TTWCGY', $app['authkey'])).'"></script>';
					}
				}
				return $synstr;
			}
	...
	function onsynlogout() {
		...
		if($this->app['synlogin']) {
			...
			foreach($this->cache['apps'] as $appid => $app) {
				if($app['synlogin'] && $app['appid'] != $this->app['appid']) {
					$synstr .= '<script type="text/javascript" src="'.$app['url'].'/api/uc.php?time='.$this->time.'&code='.urlencode($this->authcode('action=synlogout&time='.$this->time, 'TTWCGY', $app['authkey'])).'"></script>';
				}
			}
			return $synstr;
		}
