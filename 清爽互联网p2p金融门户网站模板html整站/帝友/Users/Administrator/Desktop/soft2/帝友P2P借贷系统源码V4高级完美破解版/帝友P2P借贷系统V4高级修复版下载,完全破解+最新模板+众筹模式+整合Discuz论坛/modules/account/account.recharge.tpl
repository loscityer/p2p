...
<div class="module_add">
	...
	<div class="module_border">
		...
		<div class="c">
			...
			<a href="javascript:void(0)" onclick='tipsWindown("用户详细信息查看","url:get?{$_A.admin_url}&q=code/users/view&user_id={$_A.account_recharge_result.user_id}&type=scene",700,400,"true","","true","text");'>{ $_A.account_recharge_result.username}</a>
	...
	<div class="module_border">
		...
		<div class="c">
			{if $_A.account_recharge_result.type==1}网上充值{elseif $_A.account_recharge_result.type==0}手动充值{else}线下充值{/if}
		</div>
	</div>
...
{else}
<div class="module_add">
	<div class="module_title"><strong><font color="#FF0000">{$magic.request.username} </font>充值记录</strong><div style="float:right"> <a href="{$_A.query_url_all}&type_e=excel&page={$magic.request.page|default:1}&username={$magic.request.username}&status={$magic.request.status}&type={$magic.request.type}">导出当前</a> <a href="{$_A.query_url_all}&type_e=excel&username={$magic.request.username}&status={$magic.request.status}&type={$magic.request.type}">导出全部</a>&nbsp;&nbsp;&nbsp;</div></div>
</div>
...
/充值奖励</td>
		...
		<tr  {if $key%2==1} class="tr2"{/if}>
			...
			<td >￥{ $item.money}</td>
			<td  nowrap="nowrap">{if $item.type==2}<font color="#FF0000">奖 </font>{/if}￥{ $item.fee}</td>
			...
			<td  nowrap="nowrap">{if $item.status==0}<a href="{$_A.query_url}/recharge&view={$item.id}"><font color="#FF0000">审核</font></a>{else}<a href="{$_A.query_url}/recharge&view={$item.id}">查看</a>{/if}</td>
		</tr>
		{ /foreach}
		<tr>
		...
		<div class="floatr">
			用户名：<input type="text" name="username" id="username" value="{$magic.request.username}"/> 类型{linkages name="type" nid="account_recharge_type" type="value" value="$magic.request.type" default="不限"} 状态<select id="status" ><option value="">全部</option><option value="1" {if $magic.request.status==1} selected="selected"{/if}>成功</option><option value="0" {if $magic.request.status=="0"} selected="selected"{/if}>审核</option><option value="2" {if $magic.request.status=="2"} selected="selected"{/if}>失败</option></select> 操作时间：<input type="text" name="dotime1" id="dotime1" value="{$magic.request.dotime1|default:"$day7"|date_format:"Y-m-d"}" size="15" onclick="change_picktime()"/> 到 <input type="text"  name="dotime2" value="{$magic.request.dotime2|default:"$nowtime"|date_format:"Y-m-d"}" id="dotime2" size="15" onclick="change_picktime()"/> <input type="button" value="搜索" / onclick="sousuo()">
		</div>
		</td>
	</tr>
		<tr align="center">
