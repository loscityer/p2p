...
$mod = in_array($_GET['mod'],array('synusername','clrnotice','synuid','clrfeed','pm','avator','ucpassword')) ? $_GET['mod'] : 'clrnotice';
...
if(in_array($mod,array('synusername','clrnotice','synuid','clrfeed','avator'))){
	if($_GET[''.$mod.'_submit'] || $step>0){
		if($mod == 'synusername'){
			...
			$query = DB::query("SELECT uid FROM ".DB::table('common_member')." WHERE avatarstatus = '0' AND uid BETWEEN '$start' AND '$nextstart'");
	...
	if($clearpms) {
		...
		DB::query("DELETE FROM ".UC_DBTABLEPRE."pm_members WHERE plid NOT IN (".dimplode($plids).")");
...
}elseif($mod == 'ucpassword'){
	if($_GET['password']){
		showformheader("plugins&pmod=maintain&cp=aboutucenter&operation=$operation&do=$do&mod=ucpassword",'submit');
		showtableheaders($toolslang['ucpassword']);
		showtablerow('', array('class="td21"'), array(
			$toolslang['ucpassword_tip'],
			'<input type="text" class="txt" name="password" value="'.$_GET['password'].'" /><input type="submit" class="btn" name="submit" value="'.$lang['submit'].'" />'
		));
		showtablefooter();
		showformfooter();
		...
		@fwrite($fp, $config);
		@fclose($fp);
		cpmsg($toolslang['success'],"action=plugins&pmod=maintain&cp=aboutucenter&operation=$operation&do=$do&mod=ucpassword",'succeed');
	}else{
		showformheader("plugins&pmod=maintain&cp=aboutucenter&operation=$operation&do=$do&mod=ucpassword",'submit');
		showtableheaders($toolslang['ucpassword']);
		showtablerow('', array('class="td21"'), array(
			$toolslang['ucpassword_tip'],
			'<input type="text" class="txt" name="password" value="" /><input type="submit" class="btn" name="submit" value="'.$lang['submit'].'" />'
		));
		showtablefooter();
		showformfooter();
	}
