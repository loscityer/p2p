...
function uc_api_input($data) {
	$s = urlencode(uc_authcode($data.'&agent='.md5($_SERVER['HTTP_USER_AGENT'])."&time=".time(), 'TTWCGY', UC_KEY));
	return $s;
}
...
function uc_fopen2($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
	...
	return uc_fopen("http://".$_SERVER['HTTP_HOST'].$url, $limit, $post, $cookie, $bysocket, $ip, $timeout, $block);
}
