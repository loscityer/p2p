		...
		if ($_POST['username'] == ''){
			...
			if (!isset($_POST['valicode']) || (isset($_POST['valicode']) && $_POST['valicode']!=$_SESSION['valicode'])){
				...
				if (!is_array($result)){
					...
					*/
					if ($_G['is_cookie'] ==1){
						setcookie(Key2Url("user_id","TWCMS"),authcode($result['user_id'].",".time(),"TTWCGY"),time()+60*60);
					}else{
						$_SESSION[Key2Url("user_id","TWCMS")] = authcode($result['user_id'].",".time(),"TTWCGY");
...
/* 用户退出 */
	...
	*/
	if ($_G['is_cookie'] ==1){
		setcookie(Key2Url("user_id","TWCMS"),"");
	}else{
		$_SESSION[Key2Url("user_id","TWCMS")] = "";
		$_SESSION['login_endtime'] = "";
		unset($_SESSION[Key2Url("user_id","TWCMS")]);
